...
void TSortTable::MergeSorter(PTTabRecord *pData, PTTabRecord *pBuf, int left, int mid, int right)
{
    ...
    {
        Efficiency++;
        if (pData[i]->GetKey() <= pData[j]->GetKey()) pBuf[k++] = pData[i++];
        else pBuf[k++] = pData[j++];
    }
...
void TSortTable::QuickSplit(PTTabRecord *data, int minIndex, int maxIndex)
{
    ...
    PTTabRecord pivot = data[(minIndex+maxIndex)/2];
    while (minIndex < maxIndex)
    {
        while (data[minIndex]->GetKey() < pivot->GetKey()) minIndex++;
        while (data[maxIndex]->GetKey() > pivot->GetKey()) maxIndex--;
...
PTDatValue TSortTable::FindRecord(TKey key) {
    ...
    if (last < 0 || pData[last]->GetKey() < key) { //может ли быть last <0;
        last++;
    }
    curPos = last;
...
bool TSortTable::InsertRecord(TKey key, PTDatValue value) {
    ...
    return TScanTable::InsertRecord(key,value);
}
